...
extension ObjCClass: ExtensibleSequence {
    ...
    public func insert(_ instanceVariable: ObjCInstanceVariable, name: String) throws {
        try class_addIvar(value, name.nullTerminatedUTF8String().value,
    ...
    public func insert(_ description: ObjCMethodDescription, _ implementation: ObjCImplementation) throws {
        try class_addMethod(value, Selector(description.name), implementation.value, description.signature.value).throwSelfIfFalse()
    }

    public func insert(_ method: ObjCMethod) throws {
        try class_addMethod(value, Selector(method.name), method.implementation.value, method.getDescription().signature.value).throwSelfIfFalse()
    }
    ...
    public func insert(_ property: ObjCProperty) throws {
        ...
        try class_addProperty(value, property.name, attributes, .init(attributes.count)).throwSelfIfFalse()
    }
    ...
    public func insert(_ protocol_: ObjCProtocol) throws {
        try class_addProtocol(value, protocol_.value).throwSelfIfFalse()
    }
...
extension ObjCClass {
    ...
    public func addMethod(named name: ObjCSelector, implementation: ObjCImplementation, signature: ObjCMethodSignature) throws {
        try class_addMethod(value, name.value, implementation.value, signature.value).throwSelfIfFalse()
    }
