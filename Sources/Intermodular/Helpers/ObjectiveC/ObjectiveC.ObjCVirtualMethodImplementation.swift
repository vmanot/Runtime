...
extension ObjCClass {
    public func replace(methodNamed selector: ObjCSelector, with implementation: ObjCVirtualMethodImplementation, preservingTo otherSelector: ObjCSelector) throws {
        try register(implementation, for: selector)
    ...
    public func replaceIfNecessary(methodNamed selector: ObjCSelector, with implementation: ObjCVirtualMethodImplementation, preservingTo otherSelector: ObjCSelector) throws {
        guard !isRegistered(implementation, for: selector) else {
            return
        }

        try replace(methodNamed: selector, with: implementation, preservingTo: otherSelector)
    }
}
